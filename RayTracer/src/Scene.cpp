...
namespace RayTracer
{
	Scene::Scene(vec3 backGroundColor) : backGroundColor(backGroundColor)
	{

	}

	void Scene::AddRenderable(shared_ptr<Renderable> renderable)
	{
	...
	{		
		...
		auto renderable = std::make_shared<Renderable>(areaLight->geometry, nullptr, areaLight);

		renderables.push_back(renderable);
	}

	bool Scene::Intersects(const Ray& ray, shared_ptr<SurfaceInteraction> interaction)
	{
		bool intersected = false;

		for (const auto& renderable : renderables)
		{
			if (renderable->Intersects(ray, interaction)) 
				intersected = true;
		}
	...
	bool Scene::Intersects(const Ray& ray)
	{
		...
		return Intersects(ray, interaction);
	}
}
