...
namespace RayTracer
{
	namespace RTMath
	{
		...
		bool RayPlaneIntersection(const Ray& ray, const float width, const float length, float &hitDistance)
		{
			...
			if ((x > width * 0.5f) || (y > length * 0.5f)) return false;
		...
		bool RaySphereIntersection(const Ray& ray, const float& radius, glm::vec3& hitNormal, float& hitDistance, bool &insideObject)
		{
			...
			{
				hitPosition = rayOrigin + rayDirection * u1;
				hitDistance = u1;
				hitNormal = hitPosition;
			}*/
		...
		{
			/*hitDistance = ray.origin.z / ray.direction.z;

			if (hitDistance <= 0) return false;

			vec3 p = ray.origin + ray.direction * hitDistance;
		...
		bool RayAABBIntersection(const Ray& ray, const vec3& minDimensions, 
			const vec3& maxDimensions, vec3 &hitNormal, float &hitDistance, bool &insideObject)
		{
			insideObject = false;
			float tmin = (minDimensions.x - ray.origin.x) / ray.direction.x;
			float tmax = (maxDimensions.x - ray.origin.x) / ray.direction.x;
			...
			float tymin = (minDimensions.y - ray.origin.y) / ray.direction.y;
			float tymax = (maxDimensions.y - ray.origin.y) / ray.direction.y;
			...
			float tzmin = (minDimensions.z - ray.origin.z) / ray.direction.z;
			float tzmax = (maxDimensions.z - ray.origin.z) / ray.direction.z;
